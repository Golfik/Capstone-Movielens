...
The parameters that have been chosen as primary ones were *userId* and *movieId*. *timestamp* and *title* parameters have been used to create additional parameters in different form (see next section). *genres* parameter has been excluded from the current analysis, but it's influence on the expected outcome (*rating*) has not been dismissed, and might be used in another iteration of this project.
...
In both cases I've made sure that *userId*, *movieId*, *year_of_release* and *rateday* appearing in test sets are also in training sets. This has been done by using semi_join function on those two sets, by this selected parameters.

## Deciding on modelling approach
